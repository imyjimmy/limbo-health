# Documentation

This directory keeps planning docs, specs, diagrams, and working notes out of the repository root.

## Layout

- `plans/` implementation guides, project plans, and test plans
- `specs/` product requirements and technical specifications
- `diagrams/` Mermaid diagrams and sequence-flow documentation
- `notes/` ad hoc research, reports, TODOs, and copy drafts

## Index

### Plans

- [Auth-API-gated patient fax worker](plans/auth-api-gated-patient-fax-worker.md)
- [Hermes compatibility validation](plans/hermes-spike-plan.md)
- [Auth refactor implementation guide](plans/limbo-health-auth-refactor.md)
- [Human-assisted crawler console plan](plans/human-assisted-crawler-console-plan.md)
- [Mobile app implementation guide](plans/limbo-health-mobile-implementation.md)
- [API integration test plan](plans/limbo-health-test-plan.md)

### Specs

- [AWS lean replacement spec](specs/aws-railway-replacement-spec.md)
- [Mobile app requirements](specs/limbo-health-mobile-requirements.md)
- [Medical repository structure](specs/medical-repo-structure.md)
- [NOSTR storage service spec](specs/nostr-storage-service-spec.md)
- [Records workflow explicit scope spec](specs/records-workflow-explicit-scope-spec.md)
- [Records request monetization spec](specs/records-request-tracking-monetization-spec.md)
- [User-assisted hospital portal login spec](specs/user-assisted-hospital-portal-login-spec.md)
- [`/scan` endpoint requirements](specs/scan-endpoint-requirements.md)

### Diagrams

- [Doctor clone staging repo flow](diagrams/doctor-clone.mermaid)
- [Doctor push and patient incorporate flow](diagrams/doctor-push-incorporate.mermaid)
- [Patient clone and pull flow](diagrams/patient-clone.mermaid)
- [Patient push-to-create flow](diagrams/patient-push-to-create.mermaid)
- [Sequence diagrams collection](diagrams/sequence-diagrams.md)

### Notes

- [Attach images and binaries TODO](notes/attach-images-binaries-todo.md)
- [Encrypted Git storage data usage report](notes/data-usage-report.md)
- [Expo Router limitations](notes/expo-router-limitations.md)
- [Ad hoc test commands](notes/test.txt)
- [Landing page wording variations](notes/wording-variations.txt)
