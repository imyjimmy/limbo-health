...
Users do not type Git commit messages directly in app UI flows.
Commit messages are generated by application logic based on action context (for example create, add entry, update, delete, share).

- Web encrypted flow: message is generated by feature code, then encrypted before `git.commit(...)`.
- React Native flow: message is generated by service-layer code and passed to `GitEngine`.
- Server stores what the client pushed; message policy is therefore a client implementation concern.

Detailed architecture note: `tasks/commit-message-lifecycle.md`
